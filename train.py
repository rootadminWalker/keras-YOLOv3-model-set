...
def main(args):
    ...
    try:
        ...
        log_dir = os.path.join('logs', f'{date_now.strftime("%Y_%m_%d_%H:%M:%S")}_TransferEp_{args.transfer_epoch}_TotalEP_{args.total_epoch}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_directory', type=str, required=False, default=None,
                        help='Logging directory for ModelCheckPoint, default is current date in logs directory')
    ...
    parser.add_argument('--unfreeze_level', type=float, required=False, default=1.,
                        help='Unfreeze level of the model in fine tuning stage. Set percentage of layers you want to unfreeze, like 0.9')
    ...
    parser.add_argument('--show_history', type=bool, required=False, default=False,
                        help="Show the model's summary at the beginning, may use the whole screen")
